...
# =============================================================================
        ...
        #Keep trying to make connections to mqtt broker and postgres
        while(True):
            try:
                    ...
                    if flag_connected == 1:
                        ...
                        #Run mode    
                            ...
                            for key, value in robot_dict.items():
                                ...
                                topic = self.topic_dict[key]  
                                
                                #Publish to server
                                client.publish(topic, mqtt_output, qos=1, retain=True)
                                print("Published to " + topic)
                    
                    #Run if MQTT broker is not connected
                    if flag_connected == 0:
                        
                        #Attempt to reconnect by looping to top of procedure
                        print("MQTT broker disconnected")
                        time.sleep(5)                    
                        print("Reattempting connection to MQTT broker...")
                        
                        #Break mqtt publishing loop so reconnection is attempted
                        break
